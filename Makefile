...
# Bare-bones Space ROS image, tagged `latest` unless MAIN_IMAGE_TAG is set.
main-image:
	docker buildx build --target image \
	  --build-arg IMAGE_VARIANT=main \
	  --tag $(IMAGE_NAME):$(MAIN_IMAGE_TAG) --load .

# Development image (install tree, dev tooling, IKOS), tagged `dev` unless
# DEV_IMAGE_TAG is set.
dev-image:
	docker buildx build --target image \
	  --build-arg IMAGE_VARIANT=dev \
	  --tag $(IMAGE_NAME):$(DEV_IMAGE_TAG) --load .
