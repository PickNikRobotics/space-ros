...
- [Buildx](https://docs.docker.com/build/concepts/overview/) (`sudo apt-get install docker-buildx`)
...
Each `make` target wraps a `docker buildx build` invocation. See the `Makefile` for
the exact commands, including the `build-test` and `generate-repos` helpers.
