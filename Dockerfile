...
# Add missing runtime dependencies. openssh-client (ssh/scp for git-over-SSH remotes)
# and less (git's default pager) are normally pulled in as apt Recommends by
# git, but --no-install-recommends drops them, so request them explicitly.
RUN apt-get update && apt-get install --no-install-recommends -y \
      less \
      libspdlog-dev \
      openssh-client \
